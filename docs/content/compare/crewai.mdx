...
| **Scheduling / webhooks** | Product-managed schedules + configured webhook triggers | External orchestrator required |
...
[Squadron](/) is a declarative framework for multi-agent workflows where agents, tools, models, the task graph, conditional branches, and budgets are defined in HCL. The product adds durable schedules, access control, and run operations around the worker runtime.
...
- [The Harness](/config/missions/harness)
