...
Input values accept native JSON types or string-encoded values, such as `false` or `"false"` for a boolean. Omit optional inputs to use their defaults; explicit `null` is rejected. The tool reports missing, unknown, and invalid inputs together before completing the source task, so the commander can correct the payload without starting another mission. A successful call starts the destination mission.
