...
Input values can use their native JSON types, such as `false` for a boolean or `2` for a number. String-encoded values remain supported for existing callers. Omit optional inputs to use their defaults; explicit `null` is rejected. The tool reports missing, unknown, and invalid inputs together before completing the source task, so the commander can correct the payload without starting another mission. A successful call is a real handoff, not a validation-only operation.
