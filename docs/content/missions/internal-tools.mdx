...
For **cross-mission routes**, include `mission_inputs` using the destination input definitions in the routing prompt. Supply actual task results; this call starts the destination mission when successful. Values support native JSON types as well as legacy string-encoded values:
