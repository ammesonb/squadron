...
Every model in this list works with Squadron's built-in providers. Configure the credential in Command Center, allow the keys you need in a `model_provider` block, and reference them as `models.<connection_name>.<key>`. See [Model providers](/config/models) for details.

Prices are per 1M tokens, last verified April 2026.
...
An OpenAI-compatible connection can target Ollama, vLLM, llama.cpp, LM Studio, and similar servers. Configure its endpoint in Command Center, then map stable HCL keys to the exact served model identifiers.
...
- For a provider model that is not yet in the built-in registry, use an explicit key-to-model mapping as described in [Model providers](/config/models#openai-compatible-providers).
