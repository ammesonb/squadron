...
The agent calls `ask` with the question, optional choices, and optional context. The question appears in the Squadron Inbox and on any connected gateway, such as Discord. The tool blocks until a human answers or the optional timeout fires.
...
The tool returns the human's answer as a plain string for single-select / free-text, or a JSON array string for multi-select. If no Squadron product or gateway is attached, the tool returns `[no human available]` immediately so the agent can proceed without blocking.
...
See [Gateways](/config/gateways) for surfacing questions outside the Squadron product (Discord, Slack, …).
