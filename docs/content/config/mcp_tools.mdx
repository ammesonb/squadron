...
These credentials are loaded into the worker process and used by `squadron mcp login`. CLI flags (`--client-id`, `--client-secret`) override the HCL values if both are provided. Durable user and service-principal authorization will be managed by Command Center's connection-auth model.
...
6. Makes the token and refresh token available to the current worker process
...
Removes the access and refresh tokens from the current worker process.

### Token lifecycle

The worker refreshes process-local access tokens in the background before they expire. Durable MCP authorization is intentionally separate from workspace variables and will be owned by Command Center.
