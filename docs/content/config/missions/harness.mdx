...
Route options are injected into the commander's system prompt. `task_complete` requires a `route` value, the harness validates it against declared targets, and activates the branch. `send_to` does the same unconditionally for fan-out. See [Routing](/config/missions/routing).
...
Sequential iterations pass `<LEARNINGS>` blocks forward so the agent compounds knowledge across the run. See [Iteration](/config/missions/iteration).
...
- [Missions Overview](/config/missions/overview) — the mission block, inputs, execution basics
- [Tasks](/config/missions/tasks) — task fields, dependencies, structured outputs
- [Routing](/config/missions/routing) — `router` and `send_to` in depth
- [Iteration](/config/missions/iteration) — parallel and sequential dataset iteration
- [Internal Tools](/config/missions/internal-tools) — every tool the harness gives commanders and agents
