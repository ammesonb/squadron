...
Structured output is automatically captured and stored. Downstream tasks can query it using the `query_task_output` tool (see [Internal Tools](/config/missions/internal-tools)).

## Routing

Tasks can route to other tasks (or missions) after they complete. See [Routing](/config/missions/routing) for full details, including [when to use `depends_on` vs `router` vs `send_to`](/config/missions/routing#choosing-between-depends_on-router-and-send_to).
...
- [Routing](/config/missions/routing) - Conditional and unconditional routing
- [Missions Overview](/config/missions/overview) - Mission structure
