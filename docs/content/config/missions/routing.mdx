...
1. **Fan-out** — one task triggers multiple parallel branches that each do distinct work. (If they do the same work across items, use an [iterator](/config/missions/iteration) instead.)
...
- [Internal Tools](/config/missions/internal-tools) - `task_complete` routing parameters
- [Missions Overview](/config/missions/overview) - Mission structure and execution
