...
See [Routing](/config/missions/routing) for full details.
...
See [Datasets](/config/missions/datasets) for details and examples.
...
A mission with no `memories =`, no `memory { }`, and no `scratchpad = true` does NOT get the file tools — they appear only when there's somewhere to put files. See [Memory & Scratchpad](/config/missions/folders) for the full slot model and the storage paths Squadron picks.
