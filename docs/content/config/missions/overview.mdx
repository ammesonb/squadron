...
| `memories` | list | Shared memory references, e.g. `[memories.data]` (see [Memory & Scratchpad](/config/missions/folders)) |
...
- The supplied run value is treated as sensitive and is never written to HCL
- It is **masked in logs and the Squadron app** (displayed as `********`)
...
```bash
# Supply the protected value when starting the run in Command Center
```
...
[memory](/config/missions/folders) and [packets](/config/missions/packets)).
...
- **Path** — a path inside the project root. Used from the CLI:
...
- **Base64 upload** — a JSON envelope carrying the filename and content,
  for callers that have no path on the Squadron host (the Squadron product,
...
See [Internal Tools](/config/missions/internal-tools) for full details.
...
See [Datasets](/config/missions/datasets#large-result-handling) for details.
...
Downstream tasks can query this data using `query_task_output` with filtering and aggregation. See [Tasks](/config/missions/tasks#structured-output) for details.
...
See [squadron mission](/config/cli/mission#resume) for details.

## See Also

- [Tasks](/config/missions/tasks) - Task configuration and dependencies
- [Routing](/config/missions/routing) - Conditional and unconditional task routing
- [Datasets](/config/missions/datasets) - Working with data collections
- [Iteration](/config/missions/iteration) - Processing lists of items
- [Scheduling Missions](/product/schedules) - Product-managed schedules and configured webhooks
- [Internal Tools](/config/missions/internal-tools) - Commander and agent tools
