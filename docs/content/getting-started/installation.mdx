...
On a fresh directory with an interactive terminal, `engage` runs the quickstart wizard, sets up your provider and API key, generates a starter mission, and opens the local Squadron app in your browser. See [engage](/config/cli/engage) for the full walkthrough, or [Quick Start](/getting-started/quickstart) for a guided overview.
